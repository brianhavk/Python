...
for epoch in range(epochs):
    ...
    division = 1/((abs(dρ*η)**3)*((1+ζ2)**1.5))
    product = (ddη*η2*dρ*ρ2) - (ddρ*ρ2*dη*η2) - (ζ2*((dρ*η)**3)) + (ζ*((dρ*η)**3))
    γ = division*product
    ...
    plt.legend(title=r"$\lambda_O$: "+str(round(λ, epoch+adjustment)), loc="lower right")
    plt.tight_layout()
