...
#            data1.save(self.basename+'_'+fname, 'a b m n rhoa k u i ip')
    ...
    def generateSpectraPDF(self, useall=False, rlim=None,
        ...
        with PdfPages(self.basename + '-spectra.pdf') as pdf:
    ...
    def simultaneousResistivityInversion(self, meshkw={}, **kwargs):
        ...
        self.fopRes = MultiFrameModelling(ert.ERTModelling)
        self.fopRes.setData([self.data,] * nf)
        self.fopRes.setMesh(mesh)
        dataVec = np.concatenate([rhoa for rhoa in self.RHOA.T])
        errorVec = np.tile(self.data["err"], nf)
        startModel = self.fopRes.createStartModel(dataVec)
        self.invRes = pg.Inversion(fop=self.fopRes, verbose=True, debug=False)
        self.fopRes.createConstraints()
        model = self.invRes.run(dataVec, errorVec, startModel=startModel,
                                **kwargs)
        self.RES = np.reshape(model, (nf, -1)).T
        self.RESP = np.reshape(self.invRes.response, (nf, -1))
        self.pd = self.fopRes.paraDomain
        return model
    ...
    def simultaneousPhaseInversion(self, **kwargs):
        ...
        self.invIP =pg.Inversion(fop=self.fopIP, verbose=kwargs.pop("verbose",
                                                                    True))
